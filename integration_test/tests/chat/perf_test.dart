...
void main() {
  // Mobile-only: the performance run is driven on Android (FTL) and its
  // `PerfCollector` reads metrics from logcat, so it is skipped on web.
  TestBase().runPatrolTest(
    description: 'Performance: navigation cycles, scroll, chat list',
    mobileOnly: true,
    scenarioBuilder: ($, robots) => PerfScenario($, robots),
  );
}

/// Drives the navigation / scroll / chat-list performance scenarios and flushes
/// the collected metrics. Uses concrete robots (mobile-only) directly.
class PerfScenario extends BaseTestScenario {
  PerfScenario(super.$, super.robots);
