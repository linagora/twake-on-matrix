...
Future<String> fetchOidcLoginToken({
  ...
  try {
    final redirect = await _fetchOidcRedirect(
      client: client,
      matrixURL: matrixURL,
      chatURL: chatURL,
    );
    final authorizeUri = _buildAuthorizeUri(ssoURL, redirect.params);
    final form = await _fetchSsoAuthorizeForm(
      client: client,
      authorizeUri: authorizeUri,
      matrixURL: matrixURL,
    );
    final postResult = await _submitSsoCredentials(
      client: client,
      authorizeUri: authorizeUri,
      ssoURL: ssoURL,
      form: form,
    ...
    return _exchangeCodeForLoginToken(
      ...
      sessionState: postResult.sessionState,
      oidcSession: redirect.oidcSession,
      lemonldap: postResult.lemonldap,
    );
  } finally {
...
Future<void> sendMessageAsReceiver({required String message}) async {
  ...
  final client = HttpClient()..autoUncompress = true;
  try {
    final accessToken = await _loginWithMLoginToken(
      client: client,
      matrixURL: matrixURL,
      chatURL: chatURL,
...
Future<_OidcRedirect> _fetchOidcRedirect({
  ...
  final redirectUri =
      Uri.https(matrixURL, '/_matrix/client/r0/login/sso/redirect/oidc-twake', {
        'redirectUrl':
  ...
  _setCommonBrowserHeaders(request, matrixURL);
...
Uri _buildAuthorizeUri(String ssoURL, Map<String, String> params) =>
    Uri.https(ssoURL, '/oauth2/authorize', {
...
Future<String> _exchangeCodeForLoginToken({
  ...
  required String? oidcSession,
  required String? lemonldap,
}) async {
  final callbackUri = Uri.https(matrixURL, '/_synapse/client/oidc/callback', {
    'code': code,
  ...
  request.headers
    ...
    ..set(
      ...
      _joinCookies({
        ...
        'oidc_session_no_samesite': oidcSession,
        'lemonldap': lemonldap,
      }),
    );
  final response = await request.close();
...
Future<String> _loginWithMLoginToken({
  required HttpClient client,
  required String matrixURL,
  required String chatURL,
  required String loginToken,
}) async {
  final loginUri = Uri.https(matrixURL, '/_matrix/client/v3/login');
...
Future<void> _putMatrixMessage({
  ...
  final sendUri = Uri.https(
    matrixURL,
    '/_matrix/client/v3/rooms/$groupID:linagora.com/send/m.room.message/'
    '$chatURL%3A%20Chrome%20on%20Web%20-9-'
