...
class ChatGroupOpenProfileScenario extends BaseTestScenario {
  ...
  Future<void> runTestLogic() async {
    final memberMatrixId = await _openMemberRoom();
    ...
    await _verifyProfile(memberMatrixId);
  }

  Future<String> _openMemberRoom() async {
    final String title;
    final String memberMatrixId;

    if (kIsWeb) {
      title = _webSearchPhrase;
    ...
    final opened = await robots.searchViewRobot().searchAndOpenRoom(title);
    if (!opened) {
      throw Exception('Test failed: Room "$title" was not found.');
    }
    return memberMatrixId;
  }

  /// Reads the values the UI rendered, then asserts they are displayed
  /// consistently (same checks as the legacy imperative test).
  Future<void> _verifyProfile(String memberMatrixId) async {
