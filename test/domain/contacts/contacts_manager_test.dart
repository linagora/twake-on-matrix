...
void main() {
  ...
  group('ContactsManager Search Unit test', () {
    test(
        'searchContacts should call GetCombinedContactsInteractor with query and update notifier',
        () async {
      final contactStub = ContactFixtures.contact1;
      const query = 'test';

      when(mockGetCombinedContactsInteractor.execute(val: query)).thenAnswer(
        (_) => Stream.value(
          Right(GetContactsSuccess(contacts: [contactStub])),
        ),
      );

      final List<Success> successStates = [];
      contactsManager.getContactsNotifier().addListener(() {
        contactsManager.getContactsNotifier().value.fold(
              (l) => null,
              (r) => successStates.add(r),
            );
      });

      await contactsManager.searchContacts(query);

      // Wait for stream listener to process
      await Future.delayed(Duration.zero);

      verify(mockGetCombinedContactsInteractor.execute(val: query)).called(1);

      expect(successStates.last, isA<GetContactsSuccess>());
      expect(
        (successStates.last as GetContactsSuccess).contacts,
        [contactStub],
      );
    });

    test('searchContacts should handle empty results', () async {
      const query = 'empty';

      when(mockGetCombinedContactsInteractor.execute(val: query)).thenAnswer(
        (_) => Stream.value(const Left(GetContactsIsEmpty())),
      );

      final List<Failure> failureStates = [];
      contactsManager.getContactsNotifier().addListener(() {
        contactsManager.getContactsNotifier().value.fold(
              (l) => failureStates.add(l),
              (r) => null,
            );
      });
      ...
      verify(mockGetCombinedContactsInteractor.execute(val: query)).called(1);
      expect(failureStates.last, isA<GetContactsIsEmpty>());
    });

    test('searchContacts should properly cancel previous subscription',
        () async {
      const query1 = 'q1';
      const query2 = 'q2';

      when(mockGetCombinedContactsInteractor.execute(val: query1)).thenAnswer(
        (_) => Stream.value(const Right(ContactsLoading())),
      );
      when(mockGetCombinedContactsInteractor.execute(val: query2)).thenAnswer(
        (_) => Stream.value(const Right(ContactsLoading())),
      );

      await contactsManager.searchContacts(query1);
      await contactsManager.searchContacts(query2);

      verify(mockGetCombinedContactsInteractor.execute(val: query1)).called(1);
      verify(mockGetCombinedContactsInteractor.execute(val: query2)).called(1);
    });
  });
}
