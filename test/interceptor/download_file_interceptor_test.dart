...
void main() {
  group('DownloadFileInterceptor', () {
    ...
    test(
      """WHEN there are no request match the download path,
      ...
      () async {
        ...
        expect(interceptor.currentDownloads.length, 0);
      },
    );

    test(
      """WHEN there are one item in currentDownloads set,
    ...
    test(
      """WHEN there are two item in currentDownloads set,
        THEN another request comes with different path,
      ...
      () async {
        ...
        expect(interceptor.currentDownloads.length, 3);
      },
    );
    ...
    test(
      """WHEN there are another request with same duplicate request path,
    ...
    test(
      """WHEN add a duplicate request path:
    ...
    test(
      """WHEN allows new multiple download requests
            THEN allows all new multiple download requests""",
      () async {
        final options = RequestOptions(
          path: '${downloadPath}linagora.com/dDeVrZRwVXanSwyMMoxuKVmE',
        );
    ...
    test(
      """WHEN there is an error in downloading
          THEN removes element in currentDownloads on error""",
      () async {
        ...
        expect(
          interceptor.currentDownloads.contains(requestOptions.path),
          false,
        );
        expect(interceptor.currentDownloads.length, 0);
      },
    ...
    test(
      """WHEN there are successfully download requests
            THEN removes download in currentDownloads on successful response""",
      () async {
        final requestOptions = RequestOptions(
          path: '${downloadPath}success.mp3',
        );
