#!/usr/bin/env sh
# Validates the full Sentry pipeline for Android.
# Builds a release APK with debug symbols and uploads to Sentry.
...
flutter pub get
flutter build apk --dart-define=SENTRY_DSN="${SENTRY_DSN:-}" --dart-define=SENTRY_ENVIRONMENT="${SENTRY_ENVIRONMENT:-}" --release \
  --obfuscate \
...
echo "Sentry Android pipeline: OK"
