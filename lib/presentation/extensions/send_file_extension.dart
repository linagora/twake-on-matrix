  ...
  Future<String?> sendFileEventMobile(
    ...
    if (isRoomEncrypted()) {
      ...
      try {
        ...
        await tempEncryptedFile.writeAsBytes(encryptedFileInfo.data);
        uploadStreamController?.add(
      ...
      fileInfo = FileInfo(
        fileInfo.fileName,
        fileInfo.fileSize,
        bytes: await tempEncryptedFile.readAsBytes(),
      );
      if (thumbnail != null) {
        try {
          uploadStreamController?.add(
          ...
          await (tempEncryptedThumbnailFile ?? File(''))
              .writeAsBytes(encryptedThumbnail.data);
          uploadStreamController?.add(
            const Right(
              EncryptedFileState(
                isThumbnail: true,
    ...
    while (uploadResp == null ||
      ...
      try {
        ...
        if (uploadResp != null && thumbnail != null) {
          final thumbnailBytes = isRoomEncrypted()
              ? (await (tempEncryptedThumbnailFile ?? File('')).readAsBytes())
              : thumbnail.bytes;
    ...
    final blurHash = thumbnail != null
        ? await runBenchmarked(
            '_generateBlurHash',
            () => _generateBlurHashFromBytes(thumbnail!.bytes),
          )
        : null;
  ...
  Future<ImageFileInfo> convertHeicToJpgImage(
    ...
    try {
      ...
      final convertedBytes = await HeicConverter.convertToJPG(
        heicData: fileInfo.bytes,
      );

      return ImageFileInfo(
        fileInfo.fileName,
        convertedBytes.lengthInBytes,
