...
class RecoveryWordsAPI {
  final DioClient _client = getIt.get<DioClient>(instanceName: NetworkDI.tomDioClientName);

  RecoveryWordsAPI();

  Future<RecoveryWordsResponse> getRecoveryWords() async {
    final response = await _client
        .get('/_twake/recoveryWords')
  ...
  Future<bool> saveRecoveryWords(String recoveryWords) async {
    final response = await _client
        .post('/_twake/recoveryWords', data: RecoveryWordsResponse(words: recoveryWords))
        .onError((error, stackTrace) => throw Exception(error));
