...
class VWidgetWithDependencies extends VGuard {
  ...
  Future<void> beforeEnter(VRedirector vRedirector) async {
    ...
    Logs().d('VWidgetWithDependencies::beforeEnter()');
    if (beforeDI != null) {
      beforeDI!.call(vRedirector);
    }
    for (final di in dies) {
      Logs().d('VWidgetWithDependencies::di $di');
  ...
  Future<void> beforeLeave(VRedirector vRedirector,
    ...
    await Future.forEach(dies, (di) async {
      await di.unbind();
    });
  }
