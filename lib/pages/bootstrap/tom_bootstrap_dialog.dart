...
class TomBootstrapDialogState extends State<TomBootstrapDialog> {
  ...
  UploadRecoveryKeyState _uploadRecoveryKeyState = UploadRecoveryKeyState.initial; //upload success or not

  bool? _wipe;

  @override
  void initState() {
    Logs().d('TomBootstrapDialog::initState ${widget.client.accountData.toString()}');
    Logs().d('TomBootstrapDialog::initState key: ${widget.client.accountData['m.secret_storage.key.wAfpTPuzaJt1KP4cUL0xeCTtVM9ewdBY']?.toJson()}');
    Logs().d('TomBootstrapDialog::initState default key: ${widget.client.accountData['m.secret_storage.default_key']?.toJson()}');
  ...
  Widget build(BuildContext context) {
    ...
    Logs().d('TomBootstrapDialog: ${widget.client.accountData.toString()}');
    Logs().d('TomBootstrapDialog: key: ${widget.client.accountData['m.secret_storage.key.wAfpTPuzaJt1KP4cUL0xeCTtVM9ewdBY']?.toJson()}');
    Logs().d('TomBootstrapDialog: default key: ${widget.client.accountData['m.secret_storage.default_key']?.toJson()}');
    if (bootstrap.newSsssKey?.recoveryKey != null &&
        _uploadRecoveryKeyState == UploadRecoveryKeyState.initial) {
      Logs().d('TomBootstrapDialogState::build(): have recovery key and stay in initial');
  ...
  Future<void> _backUpInRecoveryVault(String? key) async {
    Logs().d('TomBootstrapDialogState::_backUpInRecoveryVault(): $key');
    if (key == null) {
      setState(() {
    ...
    await _saveRecoveryWordsInteractor.execute(key!)
      .then((either) => either
        .fold(
          (failure) => setState(() => _uploadRecoveryKeyState = UploadRecoveryKeyState.error),
