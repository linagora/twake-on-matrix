...
class ContactsInvitationController extends State<ContactsInvitation> {
  ...
  void onSelectContact(PresentationThirdPartyContact contact) {
    if (selectedContact.value == null) {
      ...
      if (selectedContact.value is PresentationPhoneNumber &&
          contact is PresentationPhoneNumber) {
        final currentPhoneContact =
            selectedContact.value as PresentationPhoneNumber;
        if (currentPhoneContact.phoneNumber != contact.phoneNumber) {
          selectedContact.value = contact;
        } else {
          selectedContact.value = null;
        }
      } else if (selectedContact.value is PresentationEmail &&
          contact is PresentationEmail) {
        final currentEmailContact = selectedContact.value as PresentationEmail;
    ...
    sendInvitationNotifier.value.fold(
      (failure) {
        if (failure is SendInvitationFailureState) {
          TwakeSnackBar.show(
            context,
            L10n.of(context)!.invitationFailedToSend,
          );
        }
      },
      (success) {
