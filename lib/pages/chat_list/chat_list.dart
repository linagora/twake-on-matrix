...
class ChatListController extends State<ChatList>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (mounted) {
        ...
        await enableBackup(Matrix.of(context).client);
      }
    });
  ...
  Future<void> enableBackup(Client client) async {
    Logs().d('ChatListController::enableBackup()');
    if (client.encryption?.keyManager.enabled == false) {
      final recoveryWords = await _getRecoveryWords();
      await TomBootstrapDialog(
        client: client,
